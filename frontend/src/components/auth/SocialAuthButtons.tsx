...
export default function SocialAuthButtons({ mode, role = 'student' }: SocialAuthButtonsProps) {
            ...
            {/* <div className="grid grid-cols-1 gap-3">
                {providers.map((provider) => (
                    <button
                        ...
                        className={`
                            w-full py-3 px-4 border-2 rounded-lg font-semibold
                            flex items-center justify-center gap-3
                            transition-all duration-200
                            disabled:opacity-50 disabled:cursor-not-allowed
                            ${provider.bgColor} ${provider.textColor} ${provider.borderColor} ${provider.hoverBg}
                            ${loadingProvider === provider.id ? 'scale-[0.98]' : 'hover:scale-[1.02]'}
                        `}
                    >
